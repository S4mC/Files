...
:::connector
    ...
    - (h=8px) c`scanf("%d", &x); // scan formatted`:<br>
        ...
        To discard a value you can use c`%*type`:
        ```c
            scanf("%d %*d %d", &a, &b); //If you write "1 2 3" then a = 1; b = 3;
        ```
        In C, the c`scanf` function saves the line break (\n) generated by pressing Enter into the buffer.
        Therefore, when you then try to read a character after another c`scanf`, that pending \n is interpreted as the entered value.
        ```c
            printf("Enter two numbers: ");
            scanf("%d %d", &a, &b);  // ENTER after the second number leaves '\n' in the buffer

            printf("Enter an operation (+, -, *, /): ");
            scanf("%c", &op); // Read that '\n' instead of the symbol
        ```
        To fix that you can:
        ```c
            scanf(" %c", &op); // The space before reading the character causes scanf to ignore any newlines, tabs, or spaces before reading the actual character (you can also put \n or %*c instead).
        ```
